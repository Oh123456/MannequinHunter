...
#include "GraphAssetEditor/SEdNode_HFSGraphEdge.h"
...
void SEdNode_HFSGraphEdge::Construct(const FArguments& InArgs, UEdNode_GenericGraphEdge* InNode)
{
	//this->GraphNode = InNode;
	//this->UpdateGraphNode();
}

bool SEdNode_HFSGraphEdge::RequiresSecondPassLayout() const
{
	return true;
}

void SEdNode_HFSGraphEdge::PerformSecondPassLayout(const TMap< UObject*, TSharedRef<SNode> >& NodeToWidgetLookup) const
{
...
void SEdNode_HFSGraphEdge::OnNameTextCommited(const FText& InText, ETextCommit::Type CommitInfo)
{
...
void SEdNode_HFSGraphEdge::UpdateGraphNode()
{
	/*InputPins.Empty();
	OutputPins.Empty();
...
void SEdNode_HFSGraphEdge::PositionBetweenTwoNodesWithOffset(const FGeometry& StartGeom, const FGeometry& EndGeom, int32 NodeIndex, int32 MaxNodes) const
{
...
FSlateColor SEdNode_HFSGraphEdge::GetEdgeColor() const
{
	//UEdNode_GenericGraphEdge* EdgeNode = CastChecked<UEdNode_GenericGraphEdge>(GraphNode);
...
const FSlateBrush* SEdNode_HFSGraphEdge::GetEdgeImage() const
{
	return FAppStyle::GetBrush("Graph.TransitionNode.Icon");
}

EVisibility SEdNode_HFSGraphEdge::GetEdgeImageVisibility() const
{
	//UEdNode_GenericGraphEdge* EdgeNode = CastChecked<UEdNode_GenericGraphEdge>(GraphNode);
...
EVisibility SEdNode_HFSGraphEdge::GetEdgeTitleVisibility() const
{
	//UEdNode_GenericGraphEdge* EdgeNode = CastChecked<UEdNode_GenericGraphEdge>(GraphNode);
	//if (EdgeNode && EdgeNode->GenericGraphEdge && EdgeNode->GenericGraphEdge->bShouldDrawTitle)
	//	return EVisibility::Visible;
